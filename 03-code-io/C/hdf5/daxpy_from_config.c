...
int main() {
    ...
    // reading x and y
    read_vector_hdf5(N, fname_x, dataset_name, &px);
    read_vector_hdf5(N, fname_y, dataset_name, &py);

    daxpy(N, a, px, py); // result is in py
    printf("[info] look at first result: d[0] = %f\n", py[0]);

    // dump to file
    snprintf(filename, sizeof(filename), "%s_N%d_d.hd5", fname_prefix, N);
    dump_vector_hdf5(N, filename, dataset_name, py);
    free(px);
    free(py);
